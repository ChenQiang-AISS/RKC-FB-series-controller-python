...
def _log_data(timestamp: datetime, current_temperature: Optional[float], target_temperature: Optional[float], output_value: Optional[float]):
    ...
    try:
        ...
        logger.debug("Logged data: current_temperature=%s, target_temperature=%s, output_value=%s", current_temperature, target_temperature, output_value)
    ...
    while True:
        try:
            if rkc_manager.is_connected:
                ...
                # Update manager's current values (already done in get_status)
                # rkc_manager.current_temperature = current_temperature
                # rkc_manager.target_temperature = target_temperature
                # rkc_manager.output_value = output_value
            else:
